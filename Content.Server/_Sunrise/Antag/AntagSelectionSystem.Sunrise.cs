...
public sealed partial class AntagSelectionSystem
{
    ...
    private Dictionary<ICommonSession, float> GetSunrisePriorityPlayerPool(
        Dictionary<ICommonSession, float> playerPool,
        IEnumerable<AntagSpecifierPrototype> definitions)
    {
        var preferenceRoles = GetSunrisePreferenceRoles(definitions);
        var priorityPool = new Dictionary<ICommonSession, float>();
        foreach (var (player, weight) in playerPool)
        {
            if (!TryGetSunrisePriorityAntags(player, out var priorities))
                continue;
            ...
            foreach (var priority in priorities)
            {
                ...
                priorityPool.Add(player, weight);
                break;
            }
        }
    ...
    private bool TryGetSunrisePriorityAntags(ICommonSession player, out HashSet<string> priorities)
    {
        priorities = [];
        if (_sponsorsManager?.TryGetPriorityAntags(player.UserId, out var sponsorPriorities) != true ||
            sponsorPriorities == null)
            return false;

        priorities.UnionWith(sponsorPriorities);
        return priorities.Count != 0;
    }

    private static bool HasSunrisePriorityAntag(
        AntagSpecifierPrototype definition,
        HashSet<string> priorities)
    {
        foreach (var role in definition.PrefRoles)
        {
            if (priorities.Contains(role.Id))
                return true;
        }
    ...
    private IEnumerable<int> GetSunriseAntagAssignmentOrder(
        ...
        HashSet<string>? priorityAntags = null)
    {
            ...
            priority--)
        {
            for (var i = antags.Count - 1; i >= 0; i--)
            {
                ...
                if (priorityAntags != null && !HasSunrisePriorityAntag(antags[i].Definition, priorityAntags))
                    continue;
