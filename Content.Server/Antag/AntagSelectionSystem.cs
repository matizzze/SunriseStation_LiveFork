...
public sealed partial class AntagSelectionSystem : GameRuleSystem<AntagSelectionComponent>
{
    ...
    private void OnPlayerSpawning(RulePlayerSpawningEvent args)
    {
        ...
        var sunrisePreSpawnRules = GetSunriseSelectionRules(_preSpawnRules); // Sunrise-Edit
        ...
        var priorityPool = GetSunrisePriorityPlayerPool(weightedPool, _preSpawnRules.Select(rule => rule.Definition));
        while (RobustRandom.TryPickAndTake(priorityPool, out var prioritySession))
        {
            if (!TryGetSunrisePriorityAntags(prioritySession, out var priorities))
                continue;

            if (!PreAssignAntag(prioritySession, ref _preSpawnRules, out var preSelected, priorities))
            {
                if (preSelected)
                    weightedPool.Remove(prioritySession);

                continue;
            }

            weightedPool.Remove(prioritySession);
            args.PlayerPool.Remove(prioritySession);
            GameTicker.PlayerJoinGame(prioritySession);
        }
        // Sunrise edit end
        ...
        while (RobustRandom.TryPickAndTake(weightedPool, out var session))
        {
            ...
            if (!PreAssignAntag(session, ref _preSpawnRules, out _)) // Sunrise-Edit
                continue;
        ...
        RaiseSunriseAntagSelectionComplete(sunrisePreSpawnRules); // Sunrise-Edit
    ...
    private void OnJobsAssigned(RulePlayerJobsAssignedEvent args)
    {
        ...
        var sunrisePostSpawnRules = GetSunriseSelectionRules(_postSpawnRules); // Sunrise-Edit

        // Pick a random player session and then try to assign the currently available antags from it!
        // This means each player has the same chance at rolling antag, with minimal alterations to the odds by number of antags selected.
        // Sunrise edit start - сначала гарантированно пробуем назначить подходящих спонсоров.
        var weightedPool = GetWeightedPlayerPool(args.Players);
        var priorityPool = GetSunrisePriorityPlayerPool(weightedPool, _postSpawnRules.Select(rule => rule.Definition));
        while (RobustRandom.TryPickAndTake(priorityPool, out var prioritySession))
        {
            if (!TryGetSunrisePriorityAntags(prioritySession, out var priorities) ||
                !AssignAntag(prioritySession, ref _postSpawnRules, priorities))
            {
                continue;
            }
        ...
        RaiseSunriseAntagSelectionComplete(sunrisePostSpawnRules); // Sunrise-Edit
    }
    ...
    private void AssignAntags(Entity<AntagSelectionComponent> gameRule, IList<ICommonSession> players, AntagCount[] antags)
    {
        ...
        var priorityPool = GetSunrisePriorityPlayerPool(weightedPool, antags.Select(antag => antag.Definition));
        while (RobustRandom.TryPickAndTake(priorityPool, out var prioritySession))
        {
            if (!TryGetSunrisePriorityAntags(prioritySession, out var priorities) ||
                !AssignAntag(gameRule, prioritySession, ref antags, priorities))
            {
                continue;
            }
        ...
        AssignAntags(gameRule, weightedPool, antags);
        // Sunrise edit end
    }
    ...
    /// </summary>
    private bool PreAssignAntag(
        ICommonSession player,
        ref List<AntagRule> antags,
        out bool preSelected, // Sunrise-Edit — сообщает, был ли слот уже зарезервирован за игроком.
        HashSet<string>? priorityAntags = null) // Sunrise-Edit — ограничиваем приоритет выбранными спонсором антагонистами.
    {
        preSelected = false; // Sunrise-Edit
        ...
        foreach (var i in GetSunriseAntagAssignmentOrder(hasPreferences, prefs, antags, priorityAntags)) // Sunrise-Edit
        {
            var antag = antags[i];
